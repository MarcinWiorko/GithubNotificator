...
from github_notificator.models.results import Result, Discussion, ReadyForReview, ReadyForMerge, ToTested
...
class Collector:
    ...
    def _filter_task_for_another_team(self, opened_prs: list[PullRequestProxy]) -> list[PullRequestProxy]:
        return list(filter(lambda pr: all(
            another_team_thing not in pr.get_title() for another_team_thing in self.config["ignored_pr_labels"]) or any(
            label == self.config["special_label"] for label in pr.get_labels()), opened_prs, ))

    def _filter_ready_for_merge(self, opened_prs: list[PullRequestProxy]) -> list[PullRequestProxy]:
        return list(filter(lambda pr: pr.get_mergeable_state() == "clean" and pr.get_author() == self.config["me"],
                           opened_prs, ))
    ...
    def _is_ready_for_review(self, pr: PullRequestProxy) -> bool:
        return not pr.is_already_approved_by_me(self.config['me']) and pr.get_author() != self.config["me"]

    def _is_untested(self, pr: PullRequestProxy) -> bool:
        return pr.get_merged_user() == self.config["me"] and all(label != "Tested" for label in pr.get_labels())
    ...
    def _collect_ready_for_merge(self, opened_prs: list[PullRequestProxy]) -> None:
        for pr in opened_prs:
            pr_ready_to_merge = ReadyForMerge(repo=self.repo.get_name(), pr_title=pr.get_title(),
                                              pr_link=pr.get_pr_html_url(),
                                              main_branch_status=self.repo.get_main_branch_status_and_conclusion(
    ...
    def _collect_untested(self) -> None:
        ...
        for pr in untested_prs:
            if self._is_untested(pr):
                untested = ToTested(repo=self.repo.get_name(), pr_title=pr.get_title(), pr_link=pr.get_pr_html_url())
        ...
        for opened_pr in opened_prs:
            if mentions := self._filter_mentions_about_me(opened_pr):
                for mention in mentions:
                    discussion = Discussion(repo=self.repo.get_name(), pr_title=opened_pr.get_title(),
                                            pr_link=opened_pr.get_pr_html_url(), discussion_to_resolve_link=mention)
                    self.results.append(discussion)
                    print(discussion)
            elif self._is_ready_for_review(opened_pr):
                ready_for_review = ReadyForReview(repo=self.repo.get_name(), pr_title=opened_pr.get_title(),
                                                  pr_link=opened_pr.get_pr_html_url())
